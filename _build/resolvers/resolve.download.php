...
switch ($options[xPDOTransport::PACKAGE_ACTION]) {
	...
	case xPDOTransport::ACTION_UPGRADE:
		...
		if (file_exists($pnotifyPath . '.download') AND $cacheManager = $modx->getCacheManager()) {
			...
			$cacheManager->deleteTree($pnotifyPath, array_merge(array('deleteTop' => false, 'skipDirs' => false, 'extensions' => array())));
		}
		...
		if ($pnotify = $file->extract(PCLZIP_OPT_PATH, $pnotifyPath)) {
			...
			$modx->log(xPDO::LOG_LEVEL_ERROR, 'Could not extract PNotify from ' . $pnotifyZip . ' to ' . $pnotifyPath . '. Error: ' . $file->errorInfo());
